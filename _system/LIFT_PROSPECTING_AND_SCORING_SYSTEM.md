...
Per the existing V2 tracker schema (`_system/LIFT_V2_SAFE_AUTOMATION_BLUEPRINT.md`), carried forward unchanged: `company, category, website, instagram, email, contact_form_url, audit_status, outreach_status, audit_link, first_sent_date, last_touch_date, reply_state, next_action, notes`. Deliberately dropped: `score/priority/recommended_offer/draft_id/automation_notes` — V1 automation-machinery fields with no place in a manual/human-gated system. Add one field for the scoring refinement above: `score_total` (numeric, 0–24 once the sixth category is adopted) and `score_notes` (short, which categories drove the score — useful for the "why did this convert or not" learning loop in the 90-day plan).
